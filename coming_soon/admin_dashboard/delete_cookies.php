<?php 
    ...
    if (isset($_GET['id'])){
        $_SESSION["delete_cookie_id"] = $_GET['id'];
    }
    ...
    if (!isset($_SESSION["delete_cookie_id"])){
        ...
        $id = str_replace('"', "", trim(stripcslashes(stripslashes(htmlspecialchars(htmlentities(filter_var($_SESSION["delete_cookie_id"], FILTER_SANITIZE_STRING)))))));
...
<head lang="en-Us">
    ...
    <!-- All meta information for the delete cookies page -->
    <meta charset="UTF-8">
    ...
    <!-- Css files to be used on the delete cookies page -->
...
<body>
    ...
    <main id="main-body_content">
        <section>
            <div class="container">
                <div>
                    <h2 style="font-size: 1.9rem; font-weight: 600;">Delete A Cookie</h2>
                    <p>You are now about to delete a cookie.<br><b>Do you still want to go ahead with this process ?</b></p>
                    <p>
                        <form method="POST" action="<?php echo htmlentities(htmlspecialchars($_SERVER['PHP_SELF'])); ?>">
                            <button type="submit" class="btn" name="del_cookie"><I class="fa fa-check"></i> YES</button>
                        </form>
    ...
    <?php 
        if (isset($_POST['del_cookie'])){
            if (isset($_SESSION["delete_cookie_id"])){
                //Sql query to find a record in the user details table that matches up with the id in the url
                $query = $conn->prepare("
                    SELECT * FROM cookie_table
                    WHERE id = :id;
                ");
                ...
                if (count($rows) == 0){
                    ...
                    if ($query->execute()){
                        echo "
                            <script>
                              document.write(\" \");
                              window.alert(\"Cookie deleted successfully!\");
                              window.location.href = \"admin_profile.php\";
                            </script>
                        ";
                    }
    ...
    <!-- Javascript files used for the delete cookies page -->

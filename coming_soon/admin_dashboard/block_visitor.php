<?php 
    ...
    if (isset($_GET['id'])){
        $_SESSION["visitor_block_id"] = $_GET['id'];
    }
    ...
    if (!isset($_SESSION["visitor_block_id"])){
        ...
        $id = str_replace('"', "", trim(stripcslashes(stripslashes(htmlspecialchars(htmlentities(filter_var($_SESSION["visitor_block_id"], FILTER_SANITIZE_STRING)))))));
...
<head lang="en-Us">
    ...
    <!-- All meta information for the block visitor page -->
    <meta charset="UTF-8">
    ...
    <!-- Css files to be used on the block visitor page -->
...
<body>
    ...
    <main id="main-body_content">
        <section>
            <div class="container">
                <div>
                    <h2 style="font-size: 1.9rem; font-weight: 600;">Block Visitor</h2>
                    <p>You are now about to block this particular visitor.<br><b>Do you still want to go ahead with this process ?</b></p>
                    <p>
                        <form method="POST" action="<?php echo htmlentities(htmlspecialchars($_SERVER['PHP_SELF'])); ?>">
                            <button type="submit" class="btn" name="block_vis"><I class="fa fa-check"></i> BLOCK</button>
                        </form>
                        <form style="margin-top: 12px;" method="POST" action="<?php echo htmlentities(htmlspecialchars($_SERVER['PHP_SELF'])); ?>">
                            <button type="submit" class="btn" name="unblock_vis"><I class="fa fa-check"></i> UNBLOCK</button>
                        </form>
        ...
        if (isset($_POST['block_vis'])){
            if (isset($_SESSION["visitor_block_id"])){
                ...
                if (count($rows) == 0){
                    ...
                    $query = $conn->prepare("
                        SELECT user_status FROM visitor_table
                        WHERE id = :id;
                    ");
                    //--------------------------------------------------
                    ...
                    if ($query->execute()){
                        ...
                        if ($result == "Blocked"){
                            echo "
                                <script>
                                    document.write(\" \");
                                    window.alert(\"Visitor already blocked!\");
                                    window.location.href = \"admin_profile.php\";
                                </script>
                            ";
                            ...
                            if ($query->execute()){
                                echo "
                                    <script>
                                        document.write(\" \");
                                        window.alert(\"Visitor blocked successfully!\");
                                        window.location.href = \"admin_profile.php\";
                                    </script>
                                ";
                            }
                    ...
                    if ($query->execute()){
                        ...
                        if ($result == "Unblocked"){
                            echo "
                                <script>
                                    document.write(\" \");
                                    window.alert(\"Visitor already unblocked!\");
                                    window.location.href = \"admin_profile.php\";
                                </script>
                            ";
                            ...
                            if ($query->execute()){
                                echo "
                                    <script>
                                        document.write(\" \");
                                        window.alert(\"Visitor unblocked successfully!\");
                                        window.location.href = \"admin_profile.php\";
                                    </script>
                                ";
                            }
    ...
    <!-- Javascript files used for the block visitor page -->

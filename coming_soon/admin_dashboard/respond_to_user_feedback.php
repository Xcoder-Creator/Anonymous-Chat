<?php 
    ...
    if (!isset($_SESSION["feedback_id"])){
        ...
        $id = str_replace('"', "", trim(stripcslashes(stripslashes(htmlspecialchars(htmlentities(filter_var($_SESSION["feedback_id"], FILTER_SANITIZE_STRING)))))));
...
<head lang="en-Us">
    ...
    <!-- All meta information for the respond to user feedback page -->
    <meta charset="UTF-8">
    ...
    <!-- Css files to be used on the respond to user feedback page -->
...
<body>
    ...
    <main id="main-body_content">
        <section>
            <div class="container">
                <div>
                    <h2 style="font-size: 1.9rem; font-weight: 600;">Respond To User Feedback</h2>
                    <p>Using the text area below, You can compose responses and send them to users.</p>
                </div>
                <div class="row">
                    <div class="col-lg-12">
                        <div id="err_txt"></div>
                            ...
                            <textarea name="response_text" id="editor1" rows="10" cols="80">
                                <?php
                                   if(isset($_POST['response_text'])){
                                      echo $_POST['response_text'];
                                   } else {
                                      echo "Compose your response here....";
                                   }   
                                ?>
                            </textarea>
    ...
    <?php 
        if (isset($_POST['send_response'])){
            if (isset($_POST['response_text']) && strlen($_POST['response_text']) >= 60){
                $response_content = str_replace('"', "", trim(stripcslashes(stripslashes(htmlspecialchars(htmlentities(filter_var($_POST['response_text'], FILTER_SANITIZE_STRING)))))));
                try {
                    ...
                    $mail->Subject = "Anonymous-chat | Response-Daily"; //The subject of the mail to be sent
                    //-------------------------------
                    ...
                    echo "
                        <script>
                            $('#err_txt').html('<div id=\"pop_up_alert\" class=\"alert alert-success alert-dismissible\"><a href=\"javascript:void(0)\" class=\"close\" data-dismiss=\"alert\" aria-label=\"close\">&times;</a><strong>Success! </strong>Your response has been sent successfully.</div>');
                        </script>
                    ";
                } catch (Exception $e) {
                    echo "
                        <script>
                            $('#err_txt').html('<div id=\"pop_up_alert\" class=\"alert alert-warning alert-dismissible\"><a href=\"javascript:void(0)\" class=\"close\" data-dismiss=\"alert\" aria-label=\"close\">&times;</a><strong>Error! </strong>Couldn\'t send response, Check your network.</div>');
                        </script>
                    ";
                }	
            } else {
                echo "
                    <script>
                        $('#err_txt').html('<div id=\"pop_up_alert\" class=\"alert alert-danger alert-dismissible\"><a href=\"javascript:void(0)\" class=\"close\" data-dismiss=\"alert\" aria-label=\"close\">&times;</a><strong>Error! </strong>Response is to small in length to be sent.</div>');
                    </script>
                ";
            }
        }
    ...
    <!-- Javascript files used for the respond to user feedback page -->
